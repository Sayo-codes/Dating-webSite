...
import {
  getMessagesForCreator,
  createMessageAsCreator,
  MESSAGE_LIST_LIMIT,
...
export async function GET(
  ...
  const messages = await getMessagesForCreator(id, auth.creatorId, MESSAGE_LIST_LIMIT, before);
  if (messages === null)
...
export async function POST(
  ...
  const message = await createMessageAsCreator(id, auth.creatorId, {
    body: text,
